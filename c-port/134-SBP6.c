...
void fnSetChainedVector(void* ptr, UINT8 vector);
UINT8* fnGetStatusPtr(void);
static UINT8 trimFadeVal(UINT16 vol);
static UINT8 calcFadeParam(UINT8 value, UINT8 scale);
...
// for some reason, the version jumped from 21 (decimal) to 0x34 (hex)
static const UINT16 internalVersion = MAKE_VERSION(1, 0x34);
...
static const UINT8* songSeekablePosition;
...
static const char mProductName[] = "SB16 / SB Pro 2 / MCV Pro 2";
...
static       char mIOAddress[] = "\r\nDriver's I/O address set at 220 Hex\r\n";
static       char mInstalled[] = "Driver installed at INT 00H.";
static const char mRemoved[] = "\r\nDriver removed.";
static const char mSBFMDRV[] = "\r\nSBFMDRV: ";
static const char mError0[] = "Error 0000: Unknown command switch.";
static const char mError1[] = "Error 0001: SB16 / SB Pro 2 / MCV Pro 2  does not exist at the I/O address specified.";
...
static void silence(void)
{
	...
	{
		// [better but still not properly fixed] sets RL=3 for operator addr 0..8 instead of 0..15h
		writeOPL(0x80 + voice, 0x13);
	...
	// set RL=max for operator address 10h..15h
	// It completely misses operator address 09h..0Dh. (channel 3..5)
	for (reg = 0x90; reg <= 0x95; reg ++)
		writeOPL(reg, 0xFF);
...
static void setVoiceTimbre(UINT8 insID, UINT8 voiceID)
{
	...
	if (! rhythmMode || voiceID <= 6)
	{
		...
		writeOPL(reg + 0x00, *(insPtr++));
		writeOPL(reg + 0x03, *(insPtr++));
		reg += 0x20;	// register 40
		writeOPL(reg + 0x00, *(insPtr++));	// write modulator TL
		insPtr ++;	// don't write carrier TL yet (written in midiNoteOn)
		for (regSlot = 0; regSlot < 2; regSlot ++)
		{
			reg += 0x20;	// register 60/80
			writeOPL(reg + 0x00, *(insPtr++));
			writeOPL(reg + 0x03, *(insPtr++));
		}
		reg += 0x60;	// register E0
	...
	{
		//percTimbre:
		reg = voice2OpPerc[voiceID - 6] + 0x20;	// register 20
		writeOPL(reg + 0x00, *insPtr);
		insPtr += 2;
		reg += 0x20;	// register 40
		// not writing the TL register here (written in midiNoteOn)
		insPtr += 2;
		for (regSlot = 0; regSlot < 2; regSlot ++)
		{
			reg += 0x20;	// register 60/80
...
void isr08(void)
{
	if (playStatus == 1)
	{
		if (mainVolume != targetVolume)
		{
			fadeCount --;
			if (! fadeCount)
			{
				fadeCount = fadePeriod;
				doFade();
			}
		}
		waitInterval --;
...
UINT16 fnSetFade(UINT16 mainVol, UINT16 fadeVol, UINT16 volInc, UINT16 stepTicks)
{
	...
	// This function has a major issue:
	//	- all input values are clamped to [0..99]
	//	- then, they are scaled from [0..100] to [0..256]
	//	-> This means, you can't get the default/maximum volume (255),
	//	   because an input value of 99 results in 253.
	fadePeriod = stepTicks;
	fadeCount = stepTicks;
	fadeDelta = calcFadeParam(trimFadeVal(volInc), 100);
	if (mainVol != 0xFFFF)
		mainVolume = calcFadeParam(trimFadeVal(mainVol), 100);
	targetVolume = mainVolume;
	
	for (voice = 0; voice < 11; voice ++)
		voiceTimbreScaledVol[voice] = (voiceTimbreVol[voice] * mainVolume + 0x80) >> 8;
...
// ax - vol
// returns in ax (high byte is always 0)
static UINT8 trimFadeVal(UINT16 vol)
{
	if ((INT16)vol < 0)
		return 0;
...
static UINT8 calcFadeParam(UINT8 value, UINT8 scale)
{
	// scales "value" from [0..scale] to [0..0x100]
#if 1
	// port of original assembly routine
	UINT16 acc;	// al
	UINT8 bit;	// cx
	UINT8 scaled;	// dl
	...
	// The code is supposed to round up the result, based on the remainder.
	// Unfortunately, it's done incorrectly, as it should check the overflow/carry
	// bit, which it does, as well as [acc >= scale], which it does not.
	// Thus, rounding only works sometimes and it never does for [scale < 128].
	acc <<= 1;
	if (acc >= 0x100)
		scaled ++;
	return scaled;
#else
	// almost equivalent formula (same result for scale<=128)
	return ((value % scale) * 0x100) / scale;
#endif
}
...
static void doFade(void)
{
	...
	// I fading is semi-broken, as it checks only for over-/underflow of the addition/subtraction.
	// It does not compare the current volume against the target volume.
	// Thus, it will revolve around targetVolume without reaching it.
	if (mainVolume >= targetVolume)
	{
		if (fadeDelta <= mainVolume)
			mainVolume -= fadeDelta;
		else
			mainVolume = targetVolume;
	}
	else
	{
		if (mainVolume + fadeDelta < 0x100)
			mainVolume += fadeDelta;
		else
			mainVolume = targetVolume;
	}
...
void cmdInstall(void)
{
	char* mIOAddressValue;
	UINT16 val;	// ax
	UINT16 cx;
	
	mIOAddressValue = &mIOAddress[0x1E];
	val = ioBase;
	for (cx = 0; cx < 3; cx ++)
	{
		char digit = val & 0x0F;
...
static int parseEnv(void)
{
	...
	blasterVar = getenv("BLASTER");	// calls findEnvVar in the original driver
	// Note: If getenv() fails, the call to upperCase() may touch invalid memory.
	//       It should call upperCase() *after* checking that blasterVar is valid.
	upperCase(blasterVar);
	if (blasterVar == NULL)
		return 1;
	portStr = checkEnvParam('A', blasterVar);
	if (portStr == NULL)
		return 1;	// older drivers returned 2 here
