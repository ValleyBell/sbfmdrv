...
static void WriteVgmOPLCommand(UINT8 chip, UINT8 reg, UINT8 data);
...
static DEV_INFO opl2DefInf[2];
static RESMPL_STATE opl2Resmpl[2];
static DEVFUNC_WRITE_A8D8 opl2Write[2];
...
static UINT8 vgmOPLReg[2] = {0x00, 0x00};
...
static UINT32 FillBuffer(void* drvStruct, void* userParam, UINT32 bufSize, void* data)
{
	...
	Resmpl_Execute(&opl2Resmpl[0], smplCount, smplData);
	Resmpl_Execute(&opl2Resmpl[1], smplCount, smplData);
...
static UINT8 StartEmulation(void)
{
	...
	retVal = SndEmu_Start(deviceID, &devCfg, &opl2DefInf[0]);
	if (retVal)
		return retVal;
	retVal = SndEmu_Start(deviceID, &devCfg, &opl2DefInf[1]);
	
	for (curChip = 0; curChip < 2; curChip ++)
	{
		SndEmu_GetDeviceFunc(opl2DefInf[curChip].devDef, RWF_REGISTER | RWF_WRITE, DEVRW_A8D8, 0, (void**)&opl2Write[curChip]);
		
		Resmpl_SetVals(&opl2Resmpl[curChip], 0xFF, 0x100, sampleRate);
		if (curChip == 0)
			opl2Resmpl[curChip].volumeR = 0x00;
		else if (curChip == 1)
			opl2Resmpl[curChip].volumeL = 0x00;
		Resmpl_DevConnect(&opl2Resmpl[curChip], &opl2DefInf[curChip]);
		Resmpl_Init(&opl2Resmpl[curChip]);
	}
...
static UINT8 StopEmulation(void)
{
	...
	for (curChip = 0; curChip < 2; curChip ++)
	{
		Resmpl_Deinit(&opl2Resmpl[curChip]);
		SndEmu_Stop(&opl2DefInf[curChip]);
	}
	return 0x00;
}
...
void out(UINT16 port, UINT8 data)
{
	...
	case 0x43:	// PIT control
		...
		break;
		// FM ports
	case 0x220:	// chip 0, address port
	case 0x221:	// chip 0, data port
		opl2Write[0](opl2DefInf[0].dataPtr, port & 0x01, data);
		if (port & 1)
		{
			if (hFileVGM != NULL)
				WriteVgmOPLCommand(0, vgmOPLReg[0], data);
			vgmOPLReg[0] = 0x00;
		}
		else
		{
		...
		break;
	case 0x222:	// chip 1, address port
	case 0x223:	// chip 1, data port
		opl2Write[1](opl2DefInf[1].dataPtr, port & 0x01, data);
		if (port & 1)
		{
			if (hFileVGM != NULL)
				WriteVgmOPLCommand(1, vgmOPLReg[1], data);
			vgmOPLReg[1] = 0x00;
		}
		else
		{
		...
		break;
	case 0x228:	// both chips, address port
	case 0x229:	// both chips, data port
	case 0x388:
	case 0x389:
		opl2Write[0](opl2DefInf[0].dataPtr, port & 0x01, data);
		opl2Write[1](opl2DefInf[1].dataPtr, port & 0x01, data);
		if (port & 1)
		{
			...
			{
				WriteVgmOPLCommand(0, vgmOPLReg[0], data);
				WriteVgmOPLCommand(1, vgmOPLReg[0], data);
			}
			vgmOPLReg[0] = 0x00;
		}
		else
...
static void WriteVgmHeader(void)
{
	...
	vgmHdr.lngHz3812 = 3579545 | 0xC0000000;
	vgmSmplCnt = 0;
	vgmLastSmpl = 0;
	vgmPitRest = 0;
	vgmOPLReg[0] = 0x00;
