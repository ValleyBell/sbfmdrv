...
static void applyVolume(UINT8 voice, INT16 chnVolMod, INT16 vol);
...
static INT16 midiChVolumeLeft[0x10] =   { 1, -2,  4, -8,  7,-14,  2, -4,  5,-10,  8,-16,  3, -6,  6,-12};
static INT16 midiChVolumeRight[0x10] =  {-2,  1, -8,  4,-14,  7, -4,  2,-10,  5,-16,  8, -6,  3,-12,  6};
static INT16 midiChVolumeCenter[0x10] = { 1, -2,  4, -8,  7,-14,  2, -4,  5,-10,  8,-16,  3, -6,  6,-12};
...
static const char mProductName[] = "Sound Blaster Pro";
static const char mTitle[] =
	"Creative Sound Blaster FM-Driver  Version 1.30\r\n"
	...
	"\n"
	"\tSound Blaster Pro Stereo Version\n"
	"\r\n";
...
static const char mError1[] = "Error 0001: Sound Blaster Pro does not exist at the I/O address specified.";
...
static void midiNoteOn(void)
{
	...
	// Change from ioBase +8 (common left+right) to +0 (left)/+2 (right)
	{
		UINT16 iobBak = ioBase;
		ioBase &= 0x270;
		applyVolume(voice, midiChVolumeLeft[midiCh], ax);
		ioBase += 0x02;
		applyVolume(voice, midiChVolumeRight[midiCh], ax);
		ioBase = iobBak;
	}
...
static void applyVolume(UINT8 voice, INT16 chnVolMod, INT16 vol)
{
	...
	tempVol = vol + chnVolMod;
	if (tempVol < 0)
		tempVol = 0;
	else if (tempVol > 0x3F)
		tempVol = 0x3F;
	al = (0x3F - (UINT8)tempVol) | voiceTimbreKSL[voice];
...
UINT8 initPlayer(void)
{
	ioBase = (ioBase & 0xFFF0) | 0x08;	// [NOT in the actual driver] patch port value just like measureTiming() does
