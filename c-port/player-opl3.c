...
static void WriteVgmOPLCommand(UINT8 port, UINT8 reg, UINT8 data);
...
static DEV_INFO opl3DefInf;
static RESMPL_STATE opl3Resmpl;
static DEVFUNC_WRITE_A8D8 opl3Write;
...
static UINT32 FillBuffer(void* drvStruct, void* userParam, UINT32 bufSize, void* data)
{
	...
	Resmpl_Execute(&opl3Resmpl, smplCount, smplData);
...
static UINT8 StartEmulation(void)
{
	...
	deviceID = DEVID_YMF262;
	if (deviceID == DEVID_YMF262)
		devCfg.clock *= 4;	// OPL3 uses a 14 MHz clock
	retVal = SndEmu_Start(deviceID, &devCfg, &opl3DefInf);
	if (retVal)
		return retVal;
	SndEmu_GetDeviceFunc(opl3DefInf.devDef, RWF_REGISTER | RWF_WRITE, DEVRW_A8D8, 0, (void**)&opl3Write);
	
	Resmpl_SetVals(&opl3Resmpl, 0xFF, 0x100, sampleRate);
	Resmpl_DevConnect(&opl3Resmpl, &opl3DefInf);
	Resmpl_Init(&opl3Resmpl);
...
static UINT8 StopEmulation(void)
{
	Resmpl_Deinit(&opl3Resmpl);
	SndEmu_Stop(&opl3DefInf);
	return 0x00;
}
...
void out(UINT16 port, UINT8 data)
{
	...
	case 0x43:	// PIT control
		...
		break;
		// FM ports
	case 0x220:	// address port 0
	case 0x221:	// data port
	case 0x222:	// address port 1
	case 0x223:	// data port
	case 0x228:	// address port 0
	case 0x229:	// data port
	case 0x388:	// address port 0
	case 0x389:	// data port
	case 0x38A:	// address port 1
	case 0x38B:	// data port
		opl3Write(opl3DefInf.dataPtr, port & 0x03, data);
		if (port & 1)
		{
			if (hFileVGM != NULL)
				WriteVgmOPLCommand(vgmOPLPort, vgmOPLReg, data);
			vgmOPLPort = 0;
			vgmOPLReg = 0x00;
		}
		else
