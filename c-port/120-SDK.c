...
static void restoreVectors8and9(void);
UINT16 fnLoadDriver(void);
UINT16 fnUnloadDriver(void);
UINT16 fnSetIoBase(UINT16 port);
...
static UINT8 vectorNum = 0x00;
static const UINT16 internalVersion = MAKE_VERSION(1, 20);
...
{
	UINT8 a = in(0x60);
	if (! (a & 0x80) && a != 0x83)	// [bug] should compare with 83 (decimal)
	{
